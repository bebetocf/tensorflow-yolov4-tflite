...
flags.DEFINE_string('gt', './data/kite.jpg', 'path to input gt')
...
def main(_argv):
    ...
    gt_file = open(FLAGS.gt, 'r')
    gt_b, gt_s, gt_c, gt_v = [], [], [], 0
    for l in gt_file:
        print(l)
        gt_b.append([float(x) for x in l.split(' ')[1:]])
        gt_s.append(0.9)
        gt_c.append(float(l.split(' ')[0]))
        gt_v += 1

    pred_bbox = [gt_b, gt_s, gt_c, gt_v]
    print(pred_bbox)
